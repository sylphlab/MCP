...
describe('deleteItemsTool', () => {
  ...
  it('should successfully delete multiple items permanently', async () => {
    ...
    expect(mockRm).toHaveBeenCalledWith(path.resolve(WORKSPACE_ROOT, 'file1.txt'), {
      recursive: true,
      force: true,
    });
    expect(mockRm).toHaveBeenCalledWith(path.resolve(WORKSPACE_ROOT, 'folder_to_delete'), {
      recursive: true,
      force: true,
    });
  ...
  it('should handle path validation failure (outside workspace)', async () => {
    ...
    // Act
    const result = await deleteItemsTool.execute(input, {
