...
import { cp, stat } from 'node:fs/promises'; // Use named imports
import path from 'node:path';
import { copyItemsTool } from './copyItemsTool';
...
describe('copyItemsTool', () => {
    ...
    it('should successfully copy a single file when destination does not exist', async () => {
        ...
        expect(mockCp).toHaveBeenCalledWith(path.resolve(WORKSPACE_ROOT, 'source.txt'), path.resolve(WORKSPACE_ROOT, 'dest/target.txt'), { recursive: true, force: false, errorOnExist: true } // Check options
        );
    });
    ...
    it('should handle EEXIST error when destination exists and overwrite is false', async () => {
        ...
        expect(result.results[0].suggestion).toEqual(expect.any(String));
        expect(mockCp).toHaveBeenCalledTimes(1);
        expect(mockCp).toHaveBeenCalledWith(path.resolve(WORKSPACE_ROOT, 'source.txt'), path.resolve(WORKSPACE_ROOT, 'existing.txt'), { recursive: true, force: false, errorOnExist: true });
    });
    ...
    it('should NOT fail path validation if path is outside workspace and allowOutsideWorkspace is true', async () => {
        ...
        expect(mockCp).toHaveBeenCalledWith(path.resolve(WORKSPACE_ROOT, '../outside.txt'), path.resolve(WORKSPACE_ROOT, 'dest.txt'), expect.anything());
    });
    // TODO: Add tests for:
