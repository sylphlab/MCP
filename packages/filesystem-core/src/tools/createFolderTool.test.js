...
describe('createFolderTool', () => {
    ...
    it('should successfully create a single folder', async () => {
        ...
        expect(mockMkdir).toHaveBeenCalledWith(path.resolve(WORKSPACE_ROOT, 'new/folder'), { recursive: true });
    });
    ...
    it('should return validation error for empty folderPaths array', async () => {
        ...
        // Act
        const result = await createFolderTool.execute(input, WORKSPACE_ROOT); // No options needed
        // Assert
    ...
    it('should NOT fail path validation if path is outside workspace and allowOutsideWorkspace is true', async () => {
        ...
        expect(result.results[0]?.error).toBeUndefined();
        expect(mockMkdir).toHaveBeenCalledTimes(1);
        expect(mockMkdir).toHaveBeenCalledWith(path.resolve(WORKSPACE_ROOT, '../outside/new'), // Check resolved path
        { recursive: true });
    });
