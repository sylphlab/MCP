...
import { mkdir, access, unlink } from 'node:fs/promises'; // Import mocked functions
...
describe('downloadTool', () => {
  ...
  const getUrlAndCallback = (args: any[]): { url: string | URL | undefined, callback: ((res: IncomingMessage) => void) | undefined } => {
  ...
  beforeEach(() => {
    ...
    mockWriteStream.on = vi.fn((event, listener) => {
  ...
  it('should handle redirects successfully', async () => {
     ...
     vi.mocked(httpsGet).mockImplementationOnce((...args: any[]) => {
       ...
       } else { throw new Error('Unexpected URL in first mock call'); }
       return mockReq as unknown as ClientRequest;
     });
     // Override for second call
      vi.mocked(httpsGet).mockImplementationOnce((...args: any[]) => {
       ...
       } else { throw new Error('Unexpected URL in second mock call'); }
       return mockReq as unknown as ClientRequest;
     });
  ...
  it('should fail on https request error (network)', async () => {
    ...
    expect(result.error).toContain(`Download failed for item ${defaultItem.id} (${defaultItem.destinationPath}): Network connection refused`);
    expect(vi.mocked(pipeline)).not.toHaveBeenCalled(); // Use vi.mocked
  });
   ...
   it('should fail on file write error (pipeline)', async () => {
      ...
      expect(result.error).toContain(`Download failed for item ${defaultItem.id} (${defaultItem.destinationPath}): File write failed: Disk full`);
      expect(vi.mocked(unlink)).toHaveBeenCalledWith(resolvedPath); // Use vi.mocked
   });
   ...
   it('should fail if workspaceRoot is missing (though unlikely)', async () => {
    ...
    it('should handle multiple successful downloads', async () => {
        const item1 = { id: 'dl1', url: 'https://example.com/file1.zip', destinationPath: 'zips/file1.zip', overwrite: false };
        const item2 = { id: 'dl2', url: 'https://example.com/file2.img', destinationPath: 'images/file2.img', overwrite: true };
        ...
        vi.mocked(httpsGet)
            .mockImplementationOnce((...args: any[]) => { const { callback } = getUrlAndCallback(args); if (callback) callback(responseStream1); return mockReq as unknown as ClientRequest; })
    ...
    it('should handle mixed results (one success, one failure)', { timeout: 10000 }, async () => {
        const item1 = { id: 'ok', url: 'https://example.com/good.txt', destinationPath: 'good.txt', overwrite: false };
        const item2 = { id: 'bad', url: 'https://example.com/bad.txt', destinationPath: 'bad.txt', overwrite: false };
        const resolvedPath1 = `${workspaceRoot}/${item1.destinationPath}`;
        const resolvedPath2 = `${workspaceRoot}/${item2.destinationPath}`;
