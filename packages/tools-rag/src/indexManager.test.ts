...
describe('IndexManager', () => {
  ...
  beforeEach(() => {
    ...
    initializeSpy = vi
      .spyOn(IndexManager.prototype as any, 'initialize')
    ...
    (mockPineconeIndex.listPaginated as Mock)
      .mockClear()
      .mockResolvedValue({ vectors: [], pagination: {} });
  ...
  it('should throw error if Pinecone config is missing apiKey', async () => {
    ...
    await expect(IndexManager.create(badConfig)).rejects.toThrow(
      /Pinecone config requires apiKey and indexName/,
    );
    initSpy.mockRestore();
  });
  ...
  it('should throw error if ChromaDB config is missing embedding function', async () => {
    ...
    await expect(IndexManager.create(badConfig, undefined)).rejects.toThrow(
      /ChromaDB provider requires an embedding function/,
    );
    initSpy.mockRestore();
  });
  ...
  it('should handle ChromaDB client initialization errors', async () => {
    ...
    await expect(IndexManager.create(testConfigChroma, dummyEmbeddingFn)).rejects.toThrow(
      /IndexManager initialization failed/,
    );
  ...
  describe('ChromaDB Provider', () => {
    // Added describe block for structure
    let manager: IndexManager;
    let convertFilterSpy: any;

    beforeEach(async () => {
      // Local beforeEach to create instance
      // Spy on convertFilterToChromaWhere
      convertFilterSpy = vi
        .spyOn(chroma, 'convertFilterToChromaWhere')
        .mockImplementation((filter) => filter);
    ...
    it('should call initialize during creation', async () => {
      ...
      const initSpy = vi
        ...
        .mockResolvedValue(undefined);
      const dummyEmbeddingFn = { generate: vi.fn() } as unknown as IEmbeddingFunction;
      const _localManager = await IndexManager.create(testConfigChroma, dummyEmbeddingFn);
    ...
    it('should upsert items correctly mapping data and filtering metadata', async () => {
      const items: IndexedItem[] = [
        {
          ...
          content: 'doc1',
          vector: [0.1],
          metadata: { source: 's1', num: 1, bool: true, obj: { a: 1 } },
        },
      ...
      expect(mockCollection.upsert).toHaveBeenCalledWith({
        ...
        metadatas: [{ source: 's1', num: 1, bool: true }, { source: 's2', valid: 'yes' }, {}],
    ...
    it('should handle query errors', async () => {
      ...
      await expect(manager.queryIndex([0.1], 1)).rejects.toThrow(
    ...
    it('should handle upsert errors', async () => {
      ...
      await expect(
        manager.upsertItems([{ id: 'e1', content: 'err', vector: [0] }]),
      ).rejects.toThrow(`Upsert failed: ${upsertError.message}`);
    });

    it('should handle delete errors', async () => {
      const deleteError = new Error('Chroma delete failed');
      (mockCollection.delete as Mock).mockRejectedValue(deleteError);
      await expect(manager.deleteItems(['e1'])).rejects.toThrow(
  ...
  describe('InMemory Provider', () => {
    ...
    it('should update existing items on upsert', async () => {
      const item1: IndexedItem = {
        id: 'mem-upd',
        content: 'v1',
        vector: [1, 0],
        metadata: { v: 1 },
      };
      const item2: IndexedItem = {
        id: 'mem-upd',
        content: 'v2',
        vector: [0, 1],
        metadata: { v: 2 },
  ...
  describe('Pinecone Provider', () => {
    ...
    it('should call initialize during creation', async () => {
      ...
      const initSpy = vi
        .spyOn(IndexManager.prototype as any, 'initialize')
        .mockResolvedValue(undefined);
      const _manager = await IndexManager.create(testConfigPinecone); // Create will call mocked initialize
    ...
    it('should query items with filter conversion', async () => {
      ...
      const filter = { type: 'report', year: 2024 };
      const mockResponse = {
        matches: [{ id: 'p1', score: 0.9, metadata: { type: 'report', year: 2024 } }],
      };
      ...
      expect(mockPineconeIndex.query).toHaveBeenCalledWith({
        vector: queryVector,
        topK: topK,
        filter: { type: { $eq: 'report' }, year: { $eq: 2024 } },
    ...
    it('should get all IDs with pagination', async () => {
      ...
      expect(mockPineconeIndex.listPaginated).toHaveBeenNthCalledWith(1, {
        limit: 1000,
        paginationToken: undefined,
      });
      expect(mockPineconeIndex.listPaginated).toHaveBeenNthCalledWith(2, {
        limit: 1000,
