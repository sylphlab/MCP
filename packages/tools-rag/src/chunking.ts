...
import { type SyntaxNode, Tree } from '@lezer/common'; // Import Lezer types
...
export function chunkCodeAst(
  ...
  if (language === SupportedLanguage.Markdown) {
    const textChunks = splitTextWithOverlap(
      code,
      mergedOptions.maxChunkSize,
      mergedOptions.chunkOverlap,
    );
    return textChunks.map((textChunk: string, _idx: number) =>
      createChunk(document, textChunk, -1, -1, chunkCounter.index++, {
        language: language,
        warning: 'Fallback text splitting applied (Markdown AST deferred)',
      }),
    );
  }
  ...
  if (!language) {
    ...
    return textChunks.map((textChunk: string, _idx: number) =>
      createChunk(document, textChunk, -1, -1, chunkCounter.index++, {
        language: null,
        warning: 'Fallback text splitting applied (no language)',
      }),
    );
  }
  ...
  try {
    ...
    if (chunks.length === 0 && code.length > 0) {
      ...
      return textChunks.map((textChunk: string, _idx: number) =>
        createChunk(document, textChunk, -1, -1, chunkCounter.index++, {
          language: language,
          warning: 'Fallback text splitting applied (no AST chunks)',
        }),
      );
    }
    return chunks;
    ...
    const fallbackChunks = textChunks.map((textChunk: string, idx: number) =>
      createChunk(document, textChunk, -1, -1, chunkCounter.index++, {
        language: language,
