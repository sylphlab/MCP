...
import { Parser, Tree } from '@lezer/common'; // Import Parser type
...
describe('parsing', () => {
  ...
  describe('parseCode with Lezer', () => {
    ...
    it('should throw if language is unsupported', () => {
      ...
      expect(parseFn).toThrow(`Unsupported language for Lezer parser: ${unsupportedLang}`);
    });
    ...
    it('should throw wrapped error if Lezer parser fails', () => {
      ...
      const parseSpy = vi.spyOn(jsParser, 'parse').mockImplementation(() => {
        throw parseError;
      });
