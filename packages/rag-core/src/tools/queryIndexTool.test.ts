...
import {
  ...
  type QueryResult,
  type VectorDbConfig,
  type VectorDbConfigSchema,
  VectorDbProvider,
} from '../indexManager.js'; // Added VectorDbConfig import
...
describe('queryIndexTool', () => {
  ...
  let MockIndexManagerCreate: MockInstance<
    [config: VectorDbConfig, embeddingFn?: embedding.IEmbeddingFunction | undefined],
  ...
  beforeEach(() => {
    ...
    MockIndexManagerCreate = vi
      .spyOn(IndexManager, 'create')
    ...
    expect(result.content).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: 'text',
          text: expect.stringContaining('Found 2 relevant results'),
        }),
        expect.objectContaining({
          type: 'text',
          text: expect.stringContaining('Result 1 content'),
        }),
        expect.objectContaining({
          type: 'text',
          text: expect.stringContaining('Result 2 content'),
        }),
      ]),
    );
  ...
  it('should handle no results found', async () => {
    ...
    expect(result.success).toBe(true);
    expect(result.content).toEqual([
      { type: 'text', text: 'No relevant results found in the index.' },
    ]);
  ...
  it('should use provided embedding and vector DB configs', async () => {
    const ollamaConfig: Extract<
      z.infer<typeof embedding.EmbeddingModelConfigSchema>,
    ...
    const chromaConfig: Extract<
      z.infer<typeof VectorDbConfigSchema>,
