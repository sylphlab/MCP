// Import Lezer types
import { SyntaxNode, type Tree } from '@lezer/common';
import { Mock, afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
// Import module under test
import { type ChunkingOptions, chunkCodeAst, detectLanguage } from './chunking.js';
...
describe('Chunking Logic', () => {
  ...
  describe('chunkCodeAst', () => {
    const defaultOptions: Required<Omit<ChunkingOptions, 'metadata'>> = {
      maxChunkSize: 100,
      chunkOverlap: 10,
    };
    ...
    it('should use fallback text splitting when language is null', () => {
      const code =
        'This is a plain text document that is longer than one hundred characters to ensure that the fallback text splitting mechanism is triggered properly.';
    ...
    it('should use fallback text splitting when parseCode throws an error', () => {
      ...
      vi.mocked(parseCode).mockImplementation(() => {
        throw parseError;
      }); // Make parseCode throw
      ...
      expect(chunks[0].content).toBe(code);
      expect(chunks[0].metadata?.warning).toContain(
        `Fallback text splitting applied (parsing/chunking error: ${parseError.message})`,
      );
    ...
    it('should use fallback if code fits but is not a direct child boundary node', () => {
      ...
      const mockTree = {
        topNode: { firstChild: null, from: 0, to: code.length, type: { name: 'Script' } },
      } as Tree;
      ...
      // Corrected expected warning based on actual implementation detail
      expect(chunks[0].metadata?.warning).toContain(
        'Fallback text splitting applied (no AST chunks)',
      );
      expect(chunks[0].metadata?.language).toBe(language);
